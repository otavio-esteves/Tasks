    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <input wire:model.live="search" type="text" placeholder="Buscar secretarias..." class="border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 w-1/3">
                    ...
                    @foreach($secretariats as $secretariat)
                        <tr class="hover:bg-gray-50">
                            <td class="p-3 border-b">
                                <div class="font-semibold text-gray-900">{{ $secretariat->name }}</div>
                                <div class="text-sm text-gray-500">{{ $secretariat->description ?: 'Sem descricao informada.' }}</div>
                            </td>
                            <td class="p-3 border-b">
                                <span class="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded">
                                    {{ $secretariat->categories_count }}
                                </span>
                            </td>
                            <td class="p-3 border-b text-center">
                                <button wire:click="edit({{ $secretariat->id }})" class="text-blue-600 hover:underline mr-3">Editar</button>
                                <button wire:click="delete({{ $secretariat->id }})" wire:confirm="Tem certeza que deseja mover para a lixeira?" class="text-red-600 hover:underline">Excluir</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
