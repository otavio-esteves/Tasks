<div class="ods-reference-font h-screen flex flex-col overflow-hidden bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-100 selection:bg-slate-300 dark:selection:bg-slate-700"
     wire:ignore.self
     x-data="{ 
        sidebarOpen: false,
     ...
     x-on:ods-saved.window="if (!$wire.showCategoryModal) modalOpen = false"
     x-on:ods-modal-closed.window="if (!$wire.showCategoryModal) modalOpen = false"
            ...
            <div>
                ...
                <nav class="space-y-0.5">
                    ...
                    <a href="#"
                        ...
                        Equipes
                        <span class="ml-auto bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 py-0.5 px-2 rounded-full text-[10px] font-bold">--</span>
                    </a>
                    ...
                    <div class="max-w-3xl mx-auto">
                        ...
                        <div x-show="settingsTab === 'about'" class="space-y-6">
                            <div class="bg-slate-50 dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-center sm:items-start gap-5">
                                ...
                                <div class="flex-1">
                                    <h4 class="text-lg font-bold text-slate-900 dark:text-white">PrefeituraConnect</h4>
                                    ...
                                    <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mt-2">
                                        Sistema integrado de gestão urbana focado na eficiência e transparência dos serviços públicos municipais.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div x-show="filterOpen"
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400">Refine as ordens exibidas no painel.</p>
                        </div>
    ...
    <div x-data="{ show: false, message: '' }"
         x-on:ods-saved.window="show = true; message = 'ODS salva com sucesso!'; setTimeout(() => show = false, 2500)"
         x-on:ods-status-updated.window="show = true; message = 'Status atualizado!'; setTimeout(() => show = false, 2500)"
         x-show="show"
         x-transition:enter="transition ease-out duration-300"
    ...
    <main class="flex-1 overflow-y-auto p-6 pb-32 bg-slate-100 dark:bg-slate-950 transition-colors duration-200 custom-scrollbar">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            @forelse($serviceOrders as $ods)
                @php
                    $status = $ods->status;
                    $isCompleted = $status === \App\Domain\ServiceOrders\ServiceOrderStatus::Completed;
                    $isUrgent = (bool) $ods->is_urgent;
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 shrink-0">
                <div class="flex items-center gap-2">
                    <h2 class="text-lg font-bold text-slate-900 dark:text-white" 
                        x-text="mode === 'create' ? 'Nova Ordem de Serviço' : 'Editar ODS - ' + '{{ $form->odsId }}'"></h2>
                </div>
                <div class="flex items-center gap-2">
                ...
                <div class="space-y-4 p-6 overflow-y-auto custom-scrollbar flex-1" x-show="modalView === 'details'">
                    ...
                    <div>
                        ...
                        <div class="inline-flex w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 p-1" role="group">
                            <button type="button"
                                wire:click="selectStatus('{{ \App\Domain\ServiceOrders\ServiceOrderStatus::Pending->value }}')"
                                class="flex-1 h-8 rounded-md px-2 text-[10px] font-bold transition-all flex items-center justify-center gap-1.5 {{ $form->currentStatus === \App\Domain\ServiceOrders\ServiceOrderStatus::Pending->value ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                                <span class="h-1.5 w-1.5 rounded-full bg-amber-500"></span>
                                Pendente
                            </button>

                            <button type="button"
                                wire:click="selectStatus('{{ \App\Domain\ServiceOrders\ServiceOrderStatus::InProgress->value }}')"
                                class="flex-1 h-8 rounded-md px-2 text-[10px] font-bold transition-all flex items-center justify-center gap-1.5 {{ $form->currentStatus === \App\Domain\ServiceOrders\ServiceOrderStatus::InProgress->value ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                                <span class="h-1.5 w-1.5 rounded-full bg-blue-500"></span>
                                Em andamento
                            </button>

                            <button type="button"
                                wire:click="selectStatus('{{ \App\Domain\ServiceOrders\ServiceOrderStatus::Completed->value }}')"
                                class="flex-1 h-8 rounded-md px-2 text-[10px] font-bold transition-all flex items-center justify-center gap-1.5 {{ $form->currentStatus === \App\Domain\ServiceOrders\ServiceOrderStatus::Completed->value ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">
                    ...
                    <ul class="space-y-3 flex-1 overflow-y-auto custom-scrollbar mb-4 min-h-0 pr-2">
                        ...
                        @empty
                            <li class="flex flex-col items-center justify-center text-center px-6 py-10 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50/50 dark:bg-slate-950/20">
                                ...
                                <p class="text-xs text-slate-400">Adicione notas sobre o andamento e alterações da ordem de serviço.</p>
                            </li>
                        @endforelse
                    </ul>
                </div>
